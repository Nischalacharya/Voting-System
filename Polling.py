...
def pollpage():  # page for polling
    ...
    for i in range(len(names)):
        Radiobutton(ppage, text=names[i], value=names[i], variable=choose).grid(
            row=2+i, column=1)
...
def polls():  # mypolls
    ...
    Label(mpolls, text='Select Poll',
          font='Helvetica 12 bold').grid(row=1, column=3)
    select = ttk.Combobox(mpolls, values=pollnames,
                          state='readonly', textvariable=psel)
...
def create():
    ...
    Entry(cr, width=45, textvariable=cname).grid(
        row=3, column=2)  # candidate name
    Label(cr, text='Note: Enter the candidate names one by one by putting commas').grid(
        row=4, column=2)
...
def selpl():  # pollresults
    def results():
        ...
        if sel == '-select-':
            ...
            res.title('Results!')
            Label(res, text='Here is the Result!',
            ...
            for i in range(len(r)):
                data = r[i]
                Label(res, text=data[0]+': '+str(data[1]) +
                      ' votes').grid(row=2+i, column=1)
            Button(res, text='Project Results',
                   command=project).grid(row=2+i+1, column=2)
    ...
    sel = ttk.Combobox(pl, values=pollnames,
                       state='readonly', textvariable=sele)
...
def about():
    messagebox.showinfo('About', 'Developed by Sameer Thapa')
...
home['bg'] = '#49A'
Label(home, text='voting program made in python',
      font='Helvetica 12 bold', bg='#49A').grid(row=1, column=2)
...
Label(home, text='GitHub:https://github.com/sameer-thapa',
      bg='#49A').grid(row=6, column=2)
Label(home, text='Instagram:https://www.instagram.com/sulochaan/',
      bg='#49A').grid(row=7, column=2)
